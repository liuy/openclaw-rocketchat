...
function buildHelpText(): string {
  return [
    ...
    "| --- | --- | --- |",
    "| `!help` | 命令帮助 | 显示此帮助菜单 |",
    "| `!reset` | 重置对话 | 清空对话记忆，重新开始 |",
    "| `!new` | 新对话 | 开始新对话（可指定模型：`!new gpt-5`） |",
    "| `!model` | 切换模型 | 查看或切换 AI 模型 |",
    "| `!status` | 查看状态 | 显示 Agent、模型、会话状态 |",
    "| `!compact` | 压缩对话 | 压缩对话历史，释放上下文空间 |",
    "| `!abort` | 终止回复 | 立即终止当前 AI 回复 |",
    "",
    "**⚙️ 进阶命令：**",
    "| 命令 | 自然语言 | 说明 |",
    "| --- | --- | --- |",
    "| `!think <级别>` | 思考深度 | 设置推理深度（off/low/medium/high） |",
    "| `!agents` | 代理列表 | 查看可用 Agent 列表 |",
    "| `!usage` | 查看用量 | 显示 token 用量统计 |",
    "| `!context` | 查看上下文 | 显示上下文大小和构成 |",
    "| `!whoami` | — | 查看你的发送者身份 |",
    "| `!commands` | — | 查看完整命令列表 |",
    "",
