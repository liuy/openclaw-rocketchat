...
在 Telegram、Slack 等其他平台上，如果你用自然语言说"重置对话"，消息会经过以下完整流程：

```
你的消息 → Gateway → AI 模型（附带系统提示词 + 历史上下文）→ 模型理解意图 → 执行命令
```

这意味着每次「用自然语言触发命令」都要消耗一次完整的 AI 调用——包括系统提示词、对话历史、工具声明等大量 token，**只为了让 AI 理解你想执行一个简单的命令**。这是极大的浪费。

**本插件的做法不同。** 自然语言命令在**插件层面直接拦截和转换**，根本不经过 AI 模型：

```
你的消息 → 插件拦截（本地精确匹配）→ 直接执行命令
```

| | 其他平台 | 本插件 |
|--|---------|--------|
| 处理方式 | 发送给 AI 模型理解意图 | 插件本地精确匹配 |
| 消耗 token | ✅ 消耗大量 token（系统提示词 + 历史上下文） | ❌ 零 token 消耗 |
| 速度 | 较慢（等待 AI 响应） | 即时（毫秒级） |
| 准确性 | 取决于 AI 理解 | 100% 精确匹配 |

所以当你发送 `重置对话` 时，命令会**立即执行**，不花一个 token，不等一秒钟。
